...
Решение владельца: **делать всё**, но поэтапно отдельными PR (один большой PR непроверяем). Каждый этап = отдельная ветка/PR off `master`, со своими тестами и `flutter analyze` чистым. Этапы упорядочены по зависимостям: каждый последующий опирается на предыдущий, но каждый самодостаточен и мёржится отдельно.
...
- Каждый этап — отдельный PR против `hydall/GlazeFlutter:master`.
