...
class StudioControllerOntology {
  ...
  static const List<StudioControllerSpec> specs = <StudioControllerSpec>[
    ...
    StudioControllerSpec(
      ...
      outputContract:
          'At chat time, output a brief with target length, paragraph budget, POV/camera, style mode, sensory budget, beat structure, dialogue/action balance, opening constraint, and stopping point. No scene prose. Choose the paragraph budget by classifying the BEAT TYPE of the user\'s last turn using the rubric below, then mapping it to a budget.\n\nBEAT-TYPE RUBRIC — classify the user\'s last turn into exactly one of these, in priority order:\n'
          '1. ACTION — physical movement, travel, combat, pursuit, escape, riding/driving, object manipulation, a decision that is physically executed (mounting a bike, drawing a weapon, taking an object, putting on a helmet, entering a room). Even if accompanied by one or two lines of dialogue, the dominant content is physical. → budget 4-6 paragraphs.\n'
          '2. CONVERSATIONAL — two or more characters exchange dialogue with little or no physical progression; the scene stays in one place and the turn is mostly speech/reaction/thought. → budget 2-4 paragraphs.\n'
          '3. ATMOSPHERIC / INTROSPECTIVE — slow description, mood, inner reflection, environmental scene-setting, time/scene transitions, sensory immersion. → budget 4-6 paragraphs.\n'
          '4. DYNAMIC / MIXED — the turn blends physical action with meaningful dialogue and reaction in comparable weight (e.g. character acts AND talks through the action, or a short exchange resolves into a physical decision). → budget 3-5 paragraphs.\n\n'
          'DO NOT collapse a turn that contains physical movement, travel, object handling, or executed decisions into "conversational" just because it also has dialogue. Dialogue inside an action beat does NOT make it conversational. When in doubt between action and conversational, prefer action (4-6) over conversational (2-4) — a slightly longer action reply is less harmful than a truncated one that skips the physical beat.\n\n'
          'Always state both the chosen beat type, the budget, and a one-line note on why this tempo warrants it. You may add an optional "Options" list of 1-3 branchable structural/style approaches the final writer can pick from (describe the approach only, e.g. "open on a physical action" vs "open on a single line of dialogue"); never write ready-made prose.',
      fallbackPrompt:
          'Extract narrative mode, pacing, style, length, POV, tone, genre, and sensory budget into a concise response contract. Classify the user\'s last turn as ACTION, CONVERSATIONAL, ATMOSPHERIC, or DYNAMIC/MIXED using this rule: if the turn has physical movement, travel, object manipulation, or an executed decision, it is ACTION (budget 4-6) even when dialogue is present; if it is mostly speech with no physical progression it is CONVERSATIONAL (2-4); if slow/reflective it is ATMOSPHERIC (4-6); if it blends action and dialogue comparably it is DYNAMIC (3-5). Default target 6-8 paragraphs. When in doubt between action and conversational, prefer action. Include dialogue/action balance and where the response should stop. Do not draft the reply.',
      refreshPolicy: 'turn',
